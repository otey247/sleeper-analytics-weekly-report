...
##### <sup>Open issues for development:</sup> [![Help Wanted](https://img.shields.io/github/labels/otey247/sleeper-analytics-weekly-report/help%20wanted)](https://github.com/otey247/sleeper-analytics-weekly-report/labels/help%20wanted)
...
* Run `cd sleeper-analytics-weekly-report` to enter the project directory.
...
* Run `mkvirtualenv -p /usr/local/bin/python3 sleeper-analytics-weekly-report`.

* When the previous command is finished running, your command line prompt should now look something like this:

    ```bash
    (sleeper-analytics-weekly-report) [username@Computer 02:52:01 PM] ~/sleeper-analytics-weekly-report $
    ```
...
* Modify the path you find in the script after `cd` to point to wherever you cloned the application. You can either use the absolute path (something like `/Users/username/Projects/sleeper-analytics-weekly-report`), or a shortcut to your home directory (`~`), like `~/Documents/sleeper-analytics-weekly-report`).
...
```bash
(fantasy-football-metrics) host-machine: sleeper-analytics-weekly-report $ deactivate
```
 ...
 * Run `workon sleeper-analytics-weekly-report` (or whatever you named your virtual environment for the application).
